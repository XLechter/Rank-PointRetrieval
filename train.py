...
parser.add_argument('--loss_function', default='triplet_ranking_loss', choices=[
...
def train():
    ...
    elif cfg.LOSS_FUNCTION == 'triplet_ranking':
        loss_function = PNV_loss.triplet_ranking_loss
    else:
        loss_function = PNV_loss.triplet_loss_wrapper

    loss_function = PNV_loss.triplet_ranking_loss_wrapper
...
def train_one_epoch(model, optimizer, train_writer, loss_function, epoch):
    ...
    for i in range(len(train_file_idxs)//cfg.BATCH_NUM_QUERIES):
        ...
        #print('other_neg.shape', other_neg.shape)
